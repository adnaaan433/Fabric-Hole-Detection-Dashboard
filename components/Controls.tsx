...
const Controls: React.FC<ControlsProps> = ({ metrics, isRunning, onToggle }) => {
  ...
  return (
    <div className="w-full h-full">
      <div className="bg-dashboard-panel border-4 border-amber-700 rounded-xl p-2 shadow-[0_0_15px_rgba(180,83,9,0.3)] flex flex-row items-center justify-between gap-3 relative overflow-hidden h-full">
         ...
         <button
            onClick={onToggle}
            className={`w-1/3 h-full border-2 transition-all rounded-lg flex flex-col justify-center items-center gap-1 p-1 group shadow-lg ${
              isRunning 
          ...
          >
             {isRunning ? (
               <>
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 drop-shadow-md" viewBox="0 0 24 24" fill="currentColor">
                   <rect x="6" y="6" width="12" height="12" rx="2" />
                 </svg>
                 <span className="text-[10px] md:text-xs font-bold tracking-wider font-mono">STOP</span>
               </>
             ) : (
                <>
                 <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6 drop-shadow-md" viewBox="0 0 24 24" fill="currentColor">
                 ...
                 <span className="text-[10px] md:text-xs font-bold tracking-wider font-mono">START</span>
               </>
             )}
          </button>
         ...
         <button
          ...
          >
            ...
            <span className="text-sm md:text-base tracking-wider text-amber-100 uppercase text-center leading-tight">Download<br className="hidden lg:block"/> Report</span>
          </button>
